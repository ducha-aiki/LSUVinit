#!/usr/bin/env python
'''
If $CAFFE_ROOT is the installation folder for Caffe,
place this script into $CAFFE_ROOT/tools/extra
Use it like:
python $CAFFE_ROOT/tools/extra/lsuv.py /path/to/solver.prototxt /path/to/initialised.caffemodel LSUV
or
python $CAFFE_ROOT/tools/extra/lsuv.py /path/to/solver.prototxt /path/to/initialised.caffemodel Orthonormal
initialised.caffemodel is where the initialised model be saved to. If such file already exists, it will be
loaded and the initialisation distortion will be applied to it instead.

NOTE!
Name your activation layers like *_act*
Name your batch normalization layers like *BN*, or *bn*
(* - means anything),
so that the script wouldn't try to process the activation layers, like PReLU and get stuck.
'''
